...
REPORT_INTERVAL=5
TIME=60
PATH_TO_OLTP=/usr/local/Cellar/sysbench/1.0.16/share/sysbench/oltp_read_write.lua
...
sysbench $PATH_TO_OLTP --table-size=$SIZE_TABLES --tables=$NUM_TABLES --threads=$PREP_THREADS \
 --mysql-db=sysbench --mysql-user=root --mysql-password=$MYSQL_PWD --mysql-host=127.0.0.1 --mysql-port=16033 --db-driver=mysql prepare

sleep 5

sysbench $PATH_TO_OLTP --table-size=$SIZE_TABLES --tables=$NUM_TABLES --threads=$RUN_THREADS \
